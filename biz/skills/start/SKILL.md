---
name: start
description: This skill should be used when the user wants the business suite dashboard, guidance on where to start or what to do next — e.g. "/biz:start", "business dashboard", "what's the next step for my venture", "how do I use the biz suite", "از کجا شروع کنم", "داشبورد بیزینسم رو نشون بده", "قدم بعدی چیه", "راهنمای استفاده", "وضعیت ونچرهام". Acts as the orchestrator and living manual for all biz modules.
version: 1.0.0
---

# Biz Start — Dashboard, Router & Living Manual

The front door of the biz suite. Show where every venture stands, recommend the next move, and teach the suite interactively. Keep it conversational — short dashboard first, detail on demand. **Never dump the full manual unprompted.**
...
1. Riskiest assumptions unvalidated & no discovery done → `/biz:customers`
2. No market sizing → `/biz:market-research` (quick mode for `idea` stage, deep otherwise)
3. Market done, competitors missing → `/biz:competitors`
4. Stage `launched`+ and no PMF check → `/biz:pmf`
5. PMF weak → `/biz:customers` + `/biz:pmf` gap diagnosis (do NOT push GTM scaling on weak PMF)
6. PMF strong / stage `building`+ → `/biz:gtm`, then `/biz:model`
7. Everything done or 3+ modules done → `/biz:review` (board-style check)
...
| "Where are you?" | Route |
|---|---|
| فقط یه ایده دارم / just an idea | `/biz:new-venture` → then customers + quick market research |
...
|---|---|---|
| `/biz:new-venture` | register a venture, create workspace | «یه ایده جدید دارم: ...» |
| `/biz:market-research` | TAM/SAM/SOM sizing, trends, segments | «بازار این محصول چقدره؟» |
| `/biz:competitors` | competitor matrix, SWOT, battlecards | «رقبای این محصول رو تحلیل کن» |
| `/biz:customers` | Mom Test interviews, personas, synthesis | «گاید مصاحبه با مشتری بساز» |
| `/biz:pmf` | Sean Ellis test, PMF scorecard, pivot/persevere | «ببین به PMF رسیدیم یا نه» |
| `/biz:gtm` | positioning, channels, pricing, launch plan | «استراتژی ورود به بازار بچین» |
| `/biz:model` | Lean Canvas, CAC/LTV, projections | «اقتصاد واحد این بیزینس رو حساب کن» |
| `/biz:review` | board-style review of everything, kill/pivot/persevere | «کل ونچر رو بی‌رحمانه نقد کن» |
